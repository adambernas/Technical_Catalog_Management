--Tytuł: Aktualizacja listy zadań po zmianie nazwy wydania.
--Opis: Gdy wskazany dokument już istnieje procedura blokuje zmianę listy zadań na inny zakres co odzwierciedla prawidłową logikę aplikacji.
--		W repozytorium znajduje się 2 wersja tej procedury która umożliwia zmiany listy zadań istniejących wydań.
--Autor: Adam Bernaś
--Update: 23-02-2022
--Wersja: 1.4
...
--Blokada próby zmiany listy zadań w wygenerowanym wydaniu
...
AND	@DocumentValueId_7 = (SELECT DocumentValue 
							...
							ON DI.IdDTI = DTI.IdDTI
								WHERE IdDoc = @IdDoc AND
									  DTI.CodeDTI = 'NAME_R')
BEGIN
	--Zmienna @CheckStateName sprawdza nazwę aktualnego zakresu zadań
...
PRINT 'BŁĄD: Wydanie ' +'"'+ @DocumentValueId_7 +'"'+ ' ma już wygenerowaną listę zadań w zakresie '+'"'+ @CheckStateName +'".'
PRINT 'Aby stworzyć nową listę usuń obecne wydanie lub stwórz nowe.'
	ROLLBACK TRAN
	RETURN
END;

--Weryfikacja czy podana nowa nazwa wydania już istnieje
...
BEGIN
	PRINT 'BŁĄD: Podana nazwa wydania ' +'"'+ @DocumentValueId_7 +'"'+ ' juz istnieje'
	ROLLBACK TRAN
	RETURN
END; 
...
--Zmienna do określenia nazwy dokumentu
