--Tytu³: Aktualizacja listy zadañ po zmianie nazwy albo zakresu wydania.
--Opis: Uwaga! Mo¿liwoœæ zmiany zakresu listy zadañ istniej¹cego wydania nie odzwierciedla prawid³owej logiki aplikacji.
--		Ten wariant kodu napisany w celach treningowych do testowania ró¿nych scenariuszy.
--Autor: Adam Bernaœ
--Update: 23-02-2022
--Wersja: 1.4
...
--Weryfikacja czy podana nowa nazwa wydania ze wskazanym zakresem ju¿ istnieje
				  ...
				  JOIN dbo.DocumentTemplateItem as DTI
					 ON DI.IdDTI = DTI.IdDTI
						 WHERE IdDoc = @IdDoc AND
							   DTI.CodeDTI = 'SCOPE_R')
BEGIN
	PRINT 'B£¥D: Podana nazwa wydania ' +'"'+ @DocumentValueId_7 +'"'+ ' w takim zakresie zadañ juz istnieje'
	ROLLBACK TRAN
	RETURN
END;
...
--Informacja o zmianie listy zadañ w istniej¹cym wydaniu
							...
							JOIN dbo.DocumentTemplateItem as DTI
								ON DI.IdDTI = DTI.IdDTI
									WHERE IdDoc = @IdDoc AND
										  DTI.CodeDTI = 'NAME_R')
BEGIN
	PRINT 'Wydanie ' +'"'+ @DocumentValueId_7 +'"'+ ' ju¿ istnieje, wygenerowano now¹ listê zadañ w zakresie '+'"'+ @StateName +'".'
END;
